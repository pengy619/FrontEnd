  ...
  <div class="view" v-loading.fullscreen.lock="fullscreenLoading">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="260px" class="demo-ruleForm" size="small">
      <el-form-item label="线路名称" prop="FullName">
        <el-input type="textarea" autosize v-model="ruleForm.FullName" class="input_comm" :maxlength="100"></el-input>
      </el-form-item>
      ...
      <el-form-item label="线路类型" prop="EnumGroupLineType">
        ...
        <el-select v-model="ruleForm.EnumGroupLineType" placeholder="选择线路类型" @visible-change="TypeAjax(true,false,4)"
                   v-if="editType">
      ...
      <el-form-item label="组团方式" prop="EnumGroupLineMode">
        ...
        <el-select v-model="ruleForm.EnumGroupLineMode" placeholder="选择组团方式" @visible-change="ModeAjax" v-if="editMode">
      ...
      <el-form-item label="选择供应商" prop="SupplierId">
        <template v-if="!editSupplierId">
          <span>{{linesupplierid}}</span>
          <el-button size="mini" @click="newSupplierId">修改</el-button>
        </template>
        <el-select v-model="ruleForm.SupplierId" placeholder="选择供应商" v-if="editSupplierId">
      ...
      <el-form-item label="集合地点" prop="GatherAddressArr">
        <template v-if="!editAddress">
          <span>{{lineaddress}}</span>
          <el-button size="mini" @click="newAddress">修改</el-button>
        </template>
        <el-select v-model="ruleForm.GatherAddressArr" multiple placeholder="请选择集合地点"
                   @visible-change="TypeAjax(true,false,2)" v-if="editAddress">
          <el-option
      ...
      <el-form-item label="线路主题" prop="ThemesArr">
        <el-checkbox-group v-model="ruleForm.ThemesArr">
          <el-checkbox :label="item.id+''" :key='item.id' v-for="item in checkBoxItemTheme">{{item.paramName}}</el-checkbox>
        </el-checkbox-group>
        <span v-show="checkBoxItemTheme.length==0">暂无数据</span>
      </el-form-item>
      
      <el-form-item label='线路标签' prop="GroupLinelabelArr">
        <template v-if="!editLebelAddress">
          <span>{{lineLebeladdress}}</span>
          <el-button size="mini" @click="GroupLinelabelAddress">修改</el-button>
        </template>
        <el-select v-model="ruleForm.GroupLinelabelArr" multiple placeholder="请选择线路标签"
                   @visible-change="TypeAjax(true,false,5)" v-if="editLebelAddress">
          <el-option
  ...
  export default{
    data() {
      ...
      return {
        ...
        editType: false,
        editMode: false,
        editSupplierId: false,
        editAddress: false,
        editLebelAddress:false,
        linemode: '',
        linetype: '',
        linesupplierid: '',
        lineaddress: '',
    ...
    methods: {
      ...
      getLineInfo(){
        ...
        this.groupHttp.post('/GroupLine/LineInfo', {LineId: this.$route.query.lineId}).then(res => {
          if (res.data.isSuccess == true) {
            ...
            this.ruleForm.SupplierId = res.data.dtoObject.supplierId
            this.supplierList.forEach((item, index) => {
              if (this.ruleForm.SupplierId == item.id) {
                this.linesupplierid = item.name
              }
            })
            ...
            // }
            this.ruleForm.GroupLinelabelArr = res.data.dtoObject.tag.split(',');
            ...
            this.lineLebeladdress = res.data.dtoObject.tag;
      ...
      GroupLinelabelAddress(){
        this.editLebelAddress = true;
        this.ruleForm.TagArr = [];
        this.ruleForm.GroupLinelabelArr = [];
      },
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if (this.editLebelAddress == true) {
              this.GroupLineLabel.forEach((item, index) => {
                if (this.ruleForm.GroupLinelabelArr.indexOf(item.id) > -1 && this.ruleForm.TagArr.indexOf(item.paramName)==-1) {
...
//      限制图片
      ...
      TypeAjax(open, close, type){
        if (open == true) { 
            this.groupHttp.post('/GroupLine/GetGroupLineParamList', {ParamType: type})
              .then((res) => {
                // console.log(res)
                  if(type==2)
                  {
                  ...
                  if(type==5)
                  {
                    ...
                    // console.log(this.GroupLineLabel)
                  } 
              }).catch((e) => {
              console.log(e)
            })
