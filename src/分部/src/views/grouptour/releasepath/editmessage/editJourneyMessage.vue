<template>
    <div class="view" v-loading.fullscreen.lock="fullscreenLoading">
        ...
        <el-form :model="item" :rules="rules" ref="item" label-width="100px" class="demo-journeyForm" size="small" v-for="(item,index) in itemForm">
          ...
          <el-form-item label="交通" prop="returnTraffic">
...
//          ],
    ...
    methods: {
      getGroupLineRoute(){
        ...
        this.groupHttp.post('/GroupLine/GetGroupLineRoute',{LineId:this.$route.query.lineId})
          .then(res=>{
            if(res.data.isSuccess == true){
              this.fullscreenLoading = false;
              if(res.data.tripDays>0&&res.data.objects.length<1){
                for(let i=0;i<res.data.tripDays;i++){
                    this.addJourney(i+1);
                }
              }else{
