      ...
      <el-table size="small" algin="center" :data="order.table" style="width: 100%" :loading='table_loading'>
        ...
        <el-table-column size="small" align="center" label="改前价格">
          <template slot-scope="scope">
            {{ scope.row.costSmallItemOrderTotalAmont+scope.row.oldSupPrice}}
          </template>
        </el-table-column>
        <el-table-column size="small" align="center" label="改后价格">
          <template slot-scope="scope">
            {{ scope.row.costSmallItemOrderTotalAmont+scope.row.changeSupPrice}}
          </template>
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      handleCheck(row) { //处理审核
        ...
        this.groupHttp.post('/GroupLine/CheckChangePrice', { 'RecordId': this.checkPriceDto.RecordId, 'IsPass': this.checkPriceDto.IsPass, 'Reason': this.checkPriceDto.Reason })
          .then(res => {
