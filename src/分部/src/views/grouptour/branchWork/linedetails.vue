  ...
  <div class="view" v-loading="page_loading">
    ...
    <el-popover ref="downinfomation" palcement="left" trigger="click">
      <table style="width:200px;padding:0px; margin:none;border:none;font-size:12px;">
        <tr style="line-height:36px;" v-for="(item,index) in detailInfo.lineAttachments" :key="index">
          <td style=" border-bottom:1px solid gainsboro;padding:0 10px"><a :href='item.attachmentUrl' download=''>{{item.attachmentName}}</a>
          </td>
        </tr>
      </table>
    </el-popover>
    <el-button type="primary" size="small" class="button-download" icon="el-icon-download"
               v-popover:downinfomation>下载行程信息
    </el-button>
    <el-form>
      ...
      <el-form-item label="线路图片" size="small" label-width="120px">
        <img class="myimg spanmargin" v-for="(item,index) in detailInfo.linePictures" :key="index" :src='item'></img>
      </el-form-item>
      <el-form-item label="出发地" size="small" label-width="120px">
        <el-select class="spanmargin" placeholder="请选择" v-model="lineCityId" @change='cityChange'>
          <el-option v-for="(item,index) in detailInfo.lineSourceCities" :key="index" :label="item.cityName"
                     :value='item.productId'></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-table :data="productInfo" size="small" style="width:96%;margin-left:50px;" max-height="300" class="margin_top"
          ...
          <el-table-column label="操作" size="small" align="center" v-if="teamNo">
            <template slot-scope="scope">
              <el-button type="text" @click="cashOrderdetail(scope.row)" :disabled="!scope.row.isSetOrder">下单</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-form-item>
      <el-form-item>
        ...
        <span class="mysapn"><span class="verticals">| </span>小项目</span>
           <el-form-item>
                 <el-row :gutter="24" style='width:1000px;margin-left:20px' v-for="(item,index) in subentrylist" :key="index">
                    <el-col :span="24">
  ...
  export default {
    ...
    methods: {
      ...
      queryData(productId, size, index) {
        ...
        this.groupHttp.get("/GroupLine/GetGroupLineProductDetail?ProductId=" +
          productId +
          "&PageSize=" +
          (size || 10) +
          "&PageIndex=" +
      ...
      cashOrderdetail(data) {
        ...
        this.$router.push({name: "placeOrder", query:this.dataArr});
      },
      getPopoer(row) {
        console.log(row);
      },
      getSubentrylist(){
           ...
           _this.groupHttp.get('/GroupLineSmallItem/PartGroupLineSmallItem?LineId='+this.lineId).then(res=>{
      ...
      this.groupHttp
        ...
        .then(res => {
          if (res.data.isSuccess == true) {
            ...
            this.dataArr.confirmType = this.detailInfo.enumConfirmType;
