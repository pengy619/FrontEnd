  ...
  <div class="view">
    ...
    >
      ...
      <el-table-column label="操作" fixed="right" size="small" align="center" width="155">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="ViewProduct(scope.row)">审核</el-button>
    ...
    ></el-pagination>
    <!-- 处理审核 -->
    <!-- <el-dialog title="处理审核" :visible.sync="showAuditForm" center size="small">
      <el-form :model="auditForm">
        <el-form-item label="平台推广费" prop="price">
           <el-input type="text" v-model="auditForm.price"></el-input>
           <span>%</span>
        </el-form-item>
        <el-form-item label="说明" prop="explain">
          <el-input type="textarea" v-model="auditForm.explain"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="showAuditForm=false">取 消</el-button>
    ...
    ViewProduct(row) {
      ...
      if(row.productSource ==1){
        this.$router.push({
          ...
          query: { lineId: row.productId ,isAudit:true}
        });
      }else{
        this.$router.push({
          path: "/grouptour/lineDetailForUTour",
          query: { lineId: row.productId , lineCode: row.productNo,isAudit:true}
        });
      }
    }
  }
