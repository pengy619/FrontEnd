...
<script>
  export default{
    data() {
      ...
      return {
        ...
        rules: {
          ...
          VRUrl: [
            {required: true, type: 'array', message: '请上传视频', trigger: 'change'}
          ]
        }
      };
    },
    ...
    methods: {
      //确认规则
      comfirmRules () {
        this.groupHttp.post('TicketScenic/GetEnumConfirmType').then((res) => {
          console.log(res)
          if (res.status == 200) {
      ...
      getAttractionStarList () {
        this.groupHttp.get("DictionaryData/GetDictionaryData?Type=1006").then(res => {
      ...
      submitForm(formName, boo){
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.groupHttp.post( 'TicketScenic/Save', {
              Dto: this.ruleForm,
              Submit: boo
            }).then( (res) => {
              if( res.data.Issuccess === true){
