  ...
  <div id="box" class="view">
    ...
    <div class="table">
      <el-table size="small" algin="center" :data="order.table" style="width: 100%" :loading='table_loading'>
        ...
        <el-table-column size="small" align="center" label="改前价格" prop="oldSupPrice"></el-table-column>
        <el-table-column size="small" align="center" label="改后价格" prop="changeSupPrice">
        </el-table-column>
        <el-table-column size="small" align="center" label="改前应付" prop="oldReceiveAmount"></el-table-column>
        <el-table-column size="small" align="center" label="改后应付" prop="newReceiveAmount">
        </el-table-column>
        ...
        <el-table-column size="small" align="center" label="操作" fixed="right" width='150'>
          <template slot-scope="scope">
            <el-button type="text" size="mini" v-if="scope.row.showCheckBtn&&scope.row.status==0" @click="OpenCheckForm(scope.row)">处理审核</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      queryData(size, index) {
        ...
        index = index || 1;
        this.groupHttp
          .get("SingleProductOrder/ChangePriceOrderList?OrderNo=" + this.order.orderNo +
          "&CreateTimeBegin=" + beginDate + '&CreateTimeEnd=' + endDate
          + "&DisCompanyName=" + this.order.shopName + "&SupCompanyName=" + this.order.supplierName
      ...
      handleCheck(row) { //处理审核
        this.groupHttp.post('SingleProductOrder/CheckChangePrice', { 'RecordId': this.checkPriceDto.RecordId, 'IsPass': this.checkPriceDto.IsPass, 'Reason': this.checkPriceDto.Reason })
          .then(res => {
