<template>
	<div class="" >
		...
		<el-table size="small" :data="tableList" v-loading ='tableLoading' @selection-change="handleSelectionChange">
			...
			<el-table-column size="small" align="center" prop="name" label="床尺寸/m" ></el-table-column>
		...
		<el-dialog title="添加酒店床尺寸" :visible.sync="visible_add" @close="handleCloseAddDialog">
            <el-form :model="addForm" :rules="addFormRules" ref="addForm" label-width="120px" size="small">
                <el-form-item label="酒店床尺寸" prop="Name">
                    <el-input v-model="addForm.Name" class="input_comm" :maxlength="50" auto-complete="off"></el-input> m
                </el-form-item> 
            </el-form>
            <div slot="footer">
		...
		<el-dialog title="编辑酒店床尺寸" :visible.sync="visible_edit" @close="handleCloseEditDialog">
            <el-form :model="editForm" :rules="editFormRules" ref="editForm" label-width="120px" size="small">
                <el-form-item label="酒店床尺寸" prop="Name">
                    <el-input v-model="editForm.Name" class="input_comm" :maxlength="50" auto-complete="off"></el-input> m
                </el-form-item> 
            </el-form>
            <div slot="footer">
  ...
  export default{
    data() {
		return {
			...
			addFormRules:{
				Name: [
					{ required: true, type:'string', trigger: "blur",message:'床尺寸不能为空' },
				], 
			},
			editFormRules:{
				Name: [
    ...
    methods: {
		getInfo(){
			...
			this.groupHttp.post('/HotelParameter/GetHotelBedZizePageList',{
		...
		hadleAddSubmit(formName){
			this.$refs[formName].validate(valid => {
        		if (valid) {
					this.groupHttp.post('/HotelParameter/HotelBedZizeAddOrEdit',this.addForm)
					.then(res => {
						if (res.data.isSuccess) {
							this.$message({
								message: '添加成功',
		...
		hadleEditSubmit(formName){
			this.$refs[formName].validate(valid => {
        		if (valid) {
					this.groupHttp.post('/HotelParameter/HotelBedZizeAddOrEdit',this.editForm)
					.then(res => {
						if (res.data.isSuccess) {
							this.$message({
								message: '编辑成功',
		...
		handleDelete(row){
			this.$confirm("确定删除该床尺寸？", {
				...
				this.groupHttp.post('/HotelParameter/DeleteHotelBedZize',{HotelParamId:itemIds})
				.then(res => {
					if (res.data.isSuccess) {
						this.$message({
							message: '删除成功',
