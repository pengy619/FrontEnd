<template>
	<div class="" >
		...
		<el-table size="small" :data="tableList" v-loading ='tableLoading' @selection-change="handleSelectionChange">
			...
			<el-table-column size="small" align="center" prop="name" label="房间设施名称" ></el-table-column>
		...
		<el-dialog title="添加房间设施" :visible.sync="visible_add" @close="handleCloseAddDialog">
            <el-form :model="addForm" :rules="addFormRules" ref="addForm" label-width="120px" size="small">
                <el-form-item label="房间设施名称" prop="Name">
		...
		<el-dialog title="编辑房间设施" :visible.sync="visible_edit" @close="handleCloseEditDialog">
  ...
  export default{
    ...
    methods: {
		getInfo(){
			...
			this.groupHttp.post('/HotelParameter/GetHotelRoomFacilitiesPageList',{
		...
		handleAddSubmit(formName){
			this.$refs[formName].validate(valid => {
        		if (valid) {
					this.groupHttp.post('/HotelParameter/HotelRoomFacilitiesAddOrEdit',this.addForm)
					.then(res => {
						if (res.data.isSuccess) {
							this.$message({
								message: '添加成功',
		...
		handleEditSubmit(formName){
			this.$refs[formName].validate(valid => {
        		if (valid) {
					this.groupHttp.post('/HotelParameter/HotelRoomFacilitiesAddOrEdit',this.editForm)
					.then(res => {
						if (res.data.isSuccess) {
							this.$message({
								message: '编辑成功',
		...
		handleDelete(row){
			this.$confirm("确定删除该设施？", {
				...
				this.groupHttp.post('/HotelParameter/DeleteHotelRoomFacilities',{HotelParamId:itemIds})
				.then(res => {
					if (res.data.isSuccess) {
						this.$message({
							message: '删除成功',
