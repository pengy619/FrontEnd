<template>
	<div class="" >
		...
		<el-table size="small" :data="tableList" v-loading ='tableLoading' @selection-change="handleSelectionChange">
			...
			<el-table-column size="small" align="center" prop="name" label="床型名称" ></el-table-column>
		...
		<el-dialog title="添加酒店床型" :visible.sync="visible_add" @close="handleCloseAddDialog">
            <el-form :model="addForm" :rules="addFormRules" ref="addForm" label-width="120px" size="small">
                <el-form-item label="酒店床型名称" prop="Name">
		...
		<el-dialog title="编辑酒店床型" :visible.sync="visible_edit" @close="handleCloseEditDialog">
  ...
  export default{
    data() {
		return {
			...
			addFormRules:{
				Name: [
					{ required: true, type:'string', trigger: "blur",message:'床型名不能为空' },
				], 
			},
			editFormRules:{
				Name: [
    ...
    methods: {
		getInfo(){
			...
			this.groupHttp.post('/HotelParameter/GetHotelBedTypePageList',{
		...
		hadleAddSubmit(formName){
			this.$refs[formName].validate(valid => {
        		if (valid) {
					this.groupHttp.post('/HotelParameter/HotelBedTypeAddOrEdit',this.addForm)
					.then(res => {
						if (res.data.isSuccess) {
							this.$message({
								message: '添加成功',
		...
		hadleEditSubmit(formName){
			this.$refs[formName].validate(valid => {
        		if (valid) {
					this.groupHttp.post('/HotelParameter/HotelBedTypeAddOrEdit',this.editForm)
					.then(res => {
						if (res.data.isSuccess) {
							this.$message({
								message: '编辑成功',
		...
		handleDelete(row){
			this.$confirm("确定删除该床型？", {
				...
				this.groupHttp.post('/HotelParameter/DeleteHotelBedType',{HotelParamId:itemIds})
				.then(res => {
					if (res.data.isSuccess) {
						this.$message({
							message: '删除成功',
