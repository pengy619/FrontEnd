<template>
	<div class="view">
		...
		<el-row class="text-center margin_top">
			<el-button size="small" type="primary" @click="submitForm('roomForm')" :loading='btnLoading'>保存，下一步</el-button>
		</el-row>
		...
		<el-dialog title="添加房型" :visible.sync="dialogVisible" width="800px" @close="handleClose">
			...
			<span slot="footer" class="dialog-footer">
				...
				<el-button size="small" type="primary" @click="handleSubmit('roomForm')" :loading='btnLoading'>确 定</el-button>
			</span>
		</el-dialog>
	</div>
</template>
<script>
	export default{
		...
		methods: {
			...
			//提交，下一步
			submitForm(){
				this.$router.push({name:'hotelRestaurant',query:{hotelId:this.$route.query.hotelId}})
			},
			//添加床型
			handleAddRoomBeds(){
				let obj={
			...
			queryData(){
				...
				this.groupHttp.get("Hotel/GetHotelRoomListCreate",{
					params:{
						HotelId:this.$route.query.hotelId,
