<template>
    <div class="view">
        ...
        <div class="margin_bottom">
            ...
            <span>已付金额：{{list.orderPaidSum}}，</span>
            <span>已收金额：{{list.orderReceivedSum}}</span>
        </div>
        <el-table
            :data="list.data"
            style="width: 100%" v-loading="list.loading">
    ...
    methods: {
        ...
        handleResourceComfirme(id){
            this.$confirm('是否确认该资源?', '提示', {
                ...
                this.groupHttp.post('/HotelOrder/HotelResourceBranchConfirm',{
                    OrderId:id
                })
                .then((res)=>{
        ...
        exportExcel(){
            this.groupHttp.get('HotelOrder/GetBranchId')
            .then((res)=>{
                let downLoadUrl=process.env.WEB_API+`/HotelOrder/ExcelHotelOrderPageList?BoCompanyId=${res.data}&EnumOrderStatus=${this.query.EnumOrderStatus}&EnumPaymentStatus=${this.query.EnumPaymentStatus}&SearchDateType=${this.query.SearchDateType}&CheckInDate=${this.query.CheckInDate}&CheckOutDate=${this.query.CheckOutDate}&SearchType=${this.query.SearchType}&KeyWord=${this.query.KeyWord}&PageSize=${this.query.PageSize}&PageIndex=${this.query.PageIndex}`
