<template>
	<div class="view" v-loading="viewLoading">
		...
		<el-row :gutter="20" type="flex" class="info-row">
			...
			<el-col :span="14">航程类型：{{this.basicInfo.airlineName}} </el-col>
		</el-row>
		<!-- <el-row :gutter="20" type="flex" class="info-row">
			<el-col :span="6">
		...
		<el-table ref="voyageTable" :data="flightInfoData" highlight-current-row style="width: 100%">
		...
		<el-table :data="stockData" border style="width: 100%">
			...
			<el-table-column  label="余位">
				<template slot-scope="scope">
					10/{{scope.row.yuwei}}
			    </template>
			</el-table-column>
			<el-table-column label="正价票">
				<template slot-scope="scope">
					<div>已售：{{scope.row.normalQuantity}}</div>
					<div>预留：</div>
					<div>剩余：</div>
			    </template>
			</el-table-column>
			<el-table-column  label="甩位票">
				<template slot-scope="scope">
					<div>已售：</div>
					<div>预留：</div>
					<div>剩余：</div>
		...
		<h4>政策信息</h4>
		<el-row :gutter="20" type="flex" class="info-row">
			<el-col :span="20">政策名称：{{RefundRuleModel.ruleName||'无'}}</el-col>
		</el-row>
		<el-row :gutter="20" type="flex" class="info-row">
			<el-col :span="20">退票规则：{{enumRefundRule[RefundRuleModel.refundRule]||'无'}}</el-col>
		</el-row>
		...
		<h4>库存调配记录</h4>
		<el-table :data="logData" border style="width: 100%" max-height="290">
	...
	export default {
		data() {
			return {
				...
				logData:[],
				flightType:['国内','国外'],
				oprationType:['切位','控位','甩位','退切','回收切位','回收甩位'],
