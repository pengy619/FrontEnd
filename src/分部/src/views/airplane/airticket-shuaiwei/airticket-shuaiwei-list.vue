<template>
	<div class="view">
		...
		<el-table ref="multipleTable" :data="table.tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" v-loading="tableLoading">
			...
			<el-table-column prop="reductionHaveSaleQuantity" label="已售" >
			</el-table-column>
			<el-table-column prop="reductionHaveReservedQuantity" label="预留" >
			</el-table-column>
	...
	export default {
		...
		methods: {
			getList(){
				...
				this.airHttp.get('api/airticket/reductionlist?Id=0&PageSize=' + size + '&PageIndex=' + index + '&Airline=' + this.search.company + '&FlightNo=' + this.search.flightNo +'&DepartCityName=' + this.search.departure + '&ArriveCityName=' + this.search.destination +'&StartDate=' + this.search.startDate +'&EndDate='+this.search.endDate)
					.then((res)=>{
			...
			reservationAir(){
				...
				this.$router.push({path:`airticket-shuaiwei-book/`,query:{id:this.table.selected[0].id,operatingRecordId:this.table.selected[0].operatingRecordId}})
			},
			reservationDan(){
				if(this.table.selected.length>1){
				...
				this.$router.push({path:`reservation-shuaiwei/`,query:{id:this.table.selected[0].id,operatingRecordId:this.table.selected[0].operatingRecordId}})
				// this.$router.push('reservation-shuaiwei/'+this.table.selected[0].id)
			},
