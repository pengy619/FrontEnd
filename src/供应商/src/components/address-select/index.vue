<template>
  <el-cascader
    ...
    @active-item-change="addressItemChange"
    @change="retunResult"
    :props="props"
  ></el-cascader>
</template>
...
export default {
  ...
  methods: {
    getFirstData(val) {
      if (val == "1") {
        this.groupHttp.post(this.provinceUrl + "1").then(res => {
          let data = res.data;
          data.forEach(item => {
            item.children = [];
          });
      ...
      if (val == "2") {
        this.groupHttp.post(this.countryUrl + "0").then(res => {
          let data = res.data;
          data.forEach(item => {
            item.children = [];
          });
    ...
    addressItemChange(id) {
      if (this.dataType == "1") {
        if (id.length == 1) {
          this.groupHttp.post(this.cityUrl + id[0]).then(res => {
            let data = res.data;
      ...
      if (this.dataType == "2") {
        if (id.length == 1) {
          this.groupHttp
            .post(this.provinceUrl + id[id.length - 1])
            .then(res => {
              let data = res.data;
              data.forEach(item => {
                item.children = [];
        ...
        if (id.length == 2) {
          this.groupHttp.post(this.cityUrl + id[id.length - 1]).then(res => {
            let data = res.data;
            this.addressOptions.forEach(item => {
    ...
    retunResult() {
      ...
      let firstId = this.activeData[0],
        secondId = this.activeData[1];
