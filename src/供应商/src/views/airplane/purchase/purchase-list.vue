<template>
	<div class="view">
		...
		<div>
			...
			<el-button type="primary" size="small" :disabled="table.selected.length==0">退切</el-button>
		...
		<el-table ref="multipleTable" :data="table.tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" v-loading="tableLoading">
			...
			<el-table-column prop="airRoute" label="航线" >
			</el-table-column>
			<el-table-column prop="departProvinceName" label="出发地" >
			</el-table-column>
			<el-table-column prop="arriveProvinceName" label="目的地" >
			</el-table-column>
			<el-table-column prop="startTime" label="开航日期" >
				<template slot-scope="scope">
					{{scope.row.startTime.split('T')[0]}}
				</template>
			</el-table-column>
			<el-table-column prop="endTime" label="停航日期" >
				<template slot-scope="scope">
					{{scope.row.endTime.split('T')[0]}}
				</template>
			</el-table-column>
