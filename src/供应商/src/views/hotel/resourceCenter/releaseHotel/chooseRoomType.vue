	...
	<div class="view">
		...
		<el-form size="small" :inline="true"  label-width="100px">
            ...
            <el-form-item v-if="!isEdit">
                <el-button  type="text" size="mini"  @click='handleReChooseHotel'>重新选择</el-button>
            </el-form-item>
        </el-form>
        <el-row class="margin_bottom">
            <el-button size="small" type="primary"  @click="handleAddRoomType" >添加房型</el-button>
        </el-row>
		<el-table :data="tableList" v-loading="tableLoading">
			<el-table-column  prop="roomNO" label="房型编号" > </el-table-column>
			...
			<el-table-column  label="操作" fixed="right" width='100'>
				<template slot-scope="scope">
					<el-button type="text" size="mini" @click="handleChooseRoomType(scope.row)" v-if="scope.row.canChoose">选择</el-button>
					<el-button type="text" size="mini" @click="handleEditRoomType(scope.row)" v-if="scope.row.canEdit">编辑</el-button>
				</template>
			</el-table-column>
		</el-table>
		<el-pagination
			...
			<el-form :model="roomForm" :rules="rules" ref="roomForm" label-width="80px" class="demo-ruleForm" size="small">
				...
				<el-row type="flex" v-for="(item,index) in roomForm.hotelRoomBeds">
					...
					<el-form-item label="" label-width="20px"
						...
						:rules="{
							required: true, message: '请选择房尺寸', trigger: 'change'
						}"
					>
						<el-select v-model="item.bedSizeId" placeholder="请选择房尺寸" >
							<el-option :key="item.key"
									:label="item.name"
...
<script>
	export default{
		data() {
			...
			return {
				...
				rules: {
					...
					hasWindows:[{ type:'number',required: true, message: '请选择窗户', trigger: 'change'}],
					hasNetwork:[{ type:'boolean',required: true, message: '请选择区宽带', trigger: 'change'}],
		...
		methods: {
			//房间设施
			roomTypeInfo(row){
				let str='面积：'+row.area+'m² ，位于：'+row.floor+'层，可住：'+row.livingCount+'人，'+row.hasNetworkStr+'，'+row.hasWiFiStr;
				return str
			},
			//查询数据
			queryData(){
				this.tableLoading=true;
				this.groupHttp.get("/HotelProduct/GetChooseRoomList",{params:{
			...
			//选择房型
			handleChooseRoomType(row){
				if(this.isEdit){
					this.$router.push({name:'productSetting',query:{roomTypeId:row.id,productId:this.$route.query.productId,isEdit:this.isEdit}})
				}else{
					this.$router.push({name:'productSetting',query:{roomTypeId:row.id}})
				}
			},
			//添加房型
			handleAddRoomType(){
