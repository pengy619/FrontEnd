...
<script>
	export default{
		...
		methods: {
			...
			//获取信息
			getInfo(){
				this.groupHttp.get('/HotelProduct/GetHotelInfoForEdit',{
			...
			submitForm(formName){
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.groupHttp.post('/HotelProduct/CreateOrEditHotel', this.infoForm).then((res) => {
							if (res.data.isSuccess) {
								this.$message({
									message: '保存成功',
									type: 'success'
			...
			getHotelTheme(){
				this.groupHttp.get('/HotelProduct/GetHotelThemeSelect')
				.then((res) => {
					if(res.data.isSuccess){
			...
			getStarRate(){
				this.groupHttp.get('/HotelProduct/GetHotelStar')
				.then((res) => {
					if(res.data.isSuccess){
