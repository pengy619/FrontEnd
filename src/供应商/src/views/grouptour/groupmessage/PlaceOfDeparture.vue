        ...
        <el-table-column label="操作" width="200px">
          <template slot-scope="scope">
            ...
            <el-button
              size="mini"
              type="primary"
              @click="submitAudit(scope.row)" v-if="scope.row.canSubmit">提交审核
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="" :visible.sync="dialogFormVisible" width="800px">
      <el-form label-width="80px" size="small" :inline="true" >
        <el-form-item label="出发地">
          <address-select width="60%" :data-type="AirlineType" @result="addressResult" ref='addressSelect'
        ...
        <el-form-item label="" style='margin-left:80px;width:100%;'>
          <el-tag
    ...
    methods: {
      cancelAction() {
        ...
        this.PriceList = [
          ...
          // {priceName: "加盟结算价(元)",CurrencyType: "",PriceType: 4,Price: "",OldPrice: "",ChildPrice: ""},
      ...
      submitAudit(row){
          this.groupHttp.get('GroupLine/SubmitGroupLineProduct?productId='+row.productId)
            .then(res => {
      ...
      seleCurrency(open, close){
        if (open) {
          this.groupHttp.post('/GroupLine/GeEnumCurrencyTypeList').then(res => {
      ...
      savePrice(){
        ...
        for(var i = 0;i<this.PriceList.length;i++){
          var message = this.checkPrice(this.PriceList[i].Price) || this.checkPrice(this.PriceList[i].OldPrice)
        ...
        newPriceList.forEach(function (item) {
          item.CurrencyType = this.currency;
        }.bind(this));
        this.groupHttp.post('/GroupLine/SaveGroupLineProduct', {
          LineId: this.LineId,
      ...
      queryData(pageSize,pageIndex){
        ...
        this.groupHttp.post('/GroupLine/GetGroupLineProductList', {
          LineId: this.LineId,
      ...
      listDelete(index, data){
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          this.groupHttp.get('GroupLine/DeleteLineProduct?productId='+data.productId).then(res => {
            if (res.data.isSuccess == true) {
              this.$message({
                type: 'success',
                message: '删除成功!'
