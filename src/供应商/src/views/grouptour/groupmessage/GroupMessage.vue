<template>
  <div class="view">
    ...
    >
      <p class="lmargin_75">
        <span>线路名称</span>
        <span class="lmargin_20">{{groupForm.LineName}}</span>
      </p>
      ...
      <el-form-item label="计划出团人数">
        ...
        <el-input
          type="text"
          v-model="groupForm.FoundNumberCount"
          :disabled="groupForm.GoDumpling == 1 ? false : true"
          style="width:170px;"
        ></el-input>
      </el-form-item>
      ...
      <el-form-item label="类型" prop="AirlineType">
        ...
        <!-- <el-select
          v-model="groupForm.AirlineType"
          placeholder="请选择类型"
          @change="handleChangeAirlineType"
        >
          <el-option
      ...
      <el-form-item label="目的地" prop="TargetCityName">
        ...
        <address-select
          ...
          ref="addressSelect"
          width="width:350px"
          :data-type="groupForm.AirlineType"
      ...
      <el-form-item label="销售权限" prop="DistributionChannelIdList">
        <el-checkbox-group v-model="groupForm.DistributionChannelIdList">
      ...
      <div class="type-date-box">
        <el-row>
          <el-col :span="24">
            <el-form-item label="选择日期" label-width="140px" prop="DateRange" ref="dateForm">
                <el-date-picker
                  :clearable='false'
                  v-model="trip.dateRange"
                  @change="tripDateChange"
                  unlink-panels
                  type="daterange"
                  range-separator="至"
                ...
                ></el-date-picker>
                <el-button type="primary" @click="clearDate">重选日期</el-button>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row v-if="trip.DepartDateType==2">
          <el-col :span="24">
            <el-form-item label="每隔">
              <el-input-number v-model="trip.days" :min="0" label></el-input-number>天
        ...
        <el-row v-if="trip.DepartDateType==4">
          <el-col :span="24">
            <el-form-item label="逢">
              <el-input-number v-model="trip.days2" :min="0" :max="9" label="0-9的数字"></el-input-number>出团
        ...
        <el-row class="tagbox" v-show="oldTagData.length > 0">
          <el-col :span="24">
            <el-tag v-for="tag in newTagData" :key="tag" type="danger" style="margin:5px">{{tag}}</el-tag>
          </el-col>
        </el-row>
      </div>
    </el-form>
...
export default {
  ...
  data() {
    var checkInt = (rule, value, callback) => {
      if (!value && value != 0) {
        ...
        //} else if (
        //  toString(this.groupForm.SalesPreposeTime).indexOf(".") > 0 ||
        //  !Number.isInteger(Number(this.groupForm.SalesPreposeTime)) ||
        //  this.groupForm.SalesPreposeTime < 0
        //) {
    ...
    var checkFoundNumberCount = (rule, value, callback) => {
      if (!this.groupForm.FoundNumberCount) {
        ...
        this.groupForm.FoundNumberCount.indexOf(".") > 0 ||
        !Number.isInteger(Number(this.groupForm.FoundNumberCount)) ||
        this.groupForm.FoundNumberCount < 1
      ) {
    ...
    return {
      ...
      groupRules: {
        ...
        GoTime: [{ required: true, validator: checkGoTime, trigger: "blur" }],
        BackTime: [
  ...
  mounted() {
    ...
    this.getCurrentSupplierAreaType();
  },
  methods: {
    //获取当前范围
    getCurrentSupplierAreaType() {
      this.groupHttp
        .post("/GroupLine/GetCurrentSupplierAreaType")
        .then(res => {
          if (res.data.isSuccess == true) {
            // this.supplierAreaTypeList.push(res.data.dto);
            this.AirlineType = res.data.dto.name
            this.groupForm.AirlineType = res.data.dto.key
          } else {
    ...
    getGroupLinePlan() {
      this.groupHttp
        .get("/GroupLine/GetGroupLinePlan?lineId=" + this.groupForm.LineId)
        .then(res => {
          if (res.data.isSuccess == true) {
            ...
            // this.groupForm.GoHours= res.data.dto.goHours;
            // this.groupForm.GoMinutes= res.data.dto.goMinutes;
            // this.groupForm.BackHours= res.data.dto.backHours;
            // this.groupForm.BackMinutes= res.data.dto.backMinutes;
            ...
            if (res.data.dto.distributionChannel == null) {
              this.groupForm.DistributionChannelIdList = ["6", "7", "8"];
            } else {
            ...
            if (
              ...
              this.trip.GoBackTime = [
                new Date(0, 0, 0, goHours, goMinutes),
                new Date(0, 0, 0, backHours, backMinutes)
              ];
            }
          }
        });
    ...
    channelList() {
      this.groupHttp
        .post("/GroupLine/GetDistributionChannelList")
        .then(res => {
    ...
    onSubmit(formName, type) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.beforeSubmit();
          this.groupHttp
            .post("/GroupLine/SaveGroupLinePlan", {
              Dto: this.groupForm,
                ...
                if (type == "jump") {
                  ...
                  setTimeout(() => {
                    this.$router.push({
                      ...
                      query: { lineId: this.$route.query.lineId }
                    });
                  }, 500);
                } else {
    ...
    beforeSubmit() {
      ...
      //   if(this.commission.EncourageType==2){
    ...
    tripDateChange(val) {
      ...
      while (endTime.getTime() - startTime.getTime() >= 0) {
        ...
        this.allDate.push(dateStr);
        startTime.setDate(startTime.getDate() + 1);
      }
      this.trip.rangeAlldate = this.arrUnique(this.allDate);
      this.trip.tagData = this.arrUnique(this.allDate);
