  ...
  <div style="min-height: 100%;">
    <div class="ylt-top">
      <div class="am-fl am-padding-left-sm ylt-logo">
        <img :src="supplierCompanyLogo ||'./static/img/ylt_logo.png'" width="120">
        <span class="ylt-logo-text">{{supplierCompanyName || '云旅通'}}</span>
      </div>
      <ul class="ylt-top-icongroup">
        <li v-for="item in menus" v-bind:key="item.url">
    ...
    <div class="ylt-container">
      <div class="ylt-left-bar">
        ...
        >
          ...
          >
            <el-menu-item :index="item.url" v-show="item.childMenu.length==0">
              <i class="icon iconfont" :class="item.iconCss"></i>
              <span slot="title">{{item.menuName}}</span>
            </el-menu-item>
            <template slot="title">
              <i class="icon iconfont" :class="item.iconCss"></i>
              <span slot="title">{{item.menuName}}</span>
              <!--v-show="item.childMenu.length!=0"-->
            </template>
            <el-menu-item-group>
...
export default {
  ...
  mounted() {
    // CC-GY001
    this.GetAccountName();
    this.getSettingInfomation("SupplierCompanyLogo");
    this.getSettingInfomation("SupplierCompanyName");
    this.getSettingInfomation("SupplierCopyright");
  ...
  methods: {
    ...
    getSettingInfomation(SettingName) {
      this.groupHttp
        ...
        .then(res => {
          if (res.data.isSuccess == true) {
            if (SettingName == "SupplierCompanyName") {
              this.supplierCompanyName = res.data.message;
            }
            if (SettingName == "SupplierCopyright") {
              this.copyright = res.data.message;
            }
            if (SettingName == "SupplierCompanyLogo") {
              this.supplierCompanyLogo = res.data.message;
            }
          }
        });
    }
