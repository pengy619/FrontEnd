        ...
        <el-tabs type="border-card">
            <el-tab-pane label="景点信息">
                ...
                <div class='detail-div'>
                    <h4>景点详情</h4> 
                    <el-row><el-col><span class='lspan'>景点名称：</span><span class='rspan'>{{this.detailData.scenicName || '暂无'}}</span>（景点编号：{{this.detailData.scenicNo}}）</el-col></el-row>
                    <el-row><el-col><span class='lspan'>景点简称：</span><span class='rspan'>{{this.detailData.shortName || '暂无'}}</span></el-col></el-row>
                    <el-row><el-col><span class='lspan'>景点星级：</span><span class='rspan'>{{this.detailData.leveName || '暂无'}}</span></el-col></el-row>
                    <el-row><el-col><span class='lspan'>景点地址：</span><span class='rspan'>{{this.detailData.fullAddress || '暂无'}}</span></el-col></el-row>
                    <el-row><el-col><span class='lspan'>确认规则：</span><span class='rspan'>{{this.detailData.confirmRuleName || '暂无'}}</span></el-col></el-row>
                    <el-row><el-col><span class='lspan'>景点联系人：</span><span class='rspan'>{{this.detailData.contact || '暂无'}}</span></el-col></el-row>
                    <el-row><el-col><span class='lspan'>景点电话：</span><span class='rspan'>{{this.detailData.tel || '暂无'}}</span></el-col></el-row>
                    <el-row><el-col><span class='lspan'>景点类型：</span><span class='rspan'>{{this.detailData.scenicTypeName || '暂无'}}</span></el-col></el-row>
                    <el-row><el-col><span class='lspan'>景点经度：</span><span class='rspan'>{{this.detailData.longitude || '暂无'}}</span></el-col></el-row>
                    <el-row><el-col><span class='lspan'>景点纬度：</span><span class='rspan'>{{this.detailData.latitude || '暂无'}}</span></el-col></el-row>
                    <el-row>
                        <el-col class='w-100'>
    ...
    export default{
        ...
        methods:{
            ...
            queryData (ticketScenicId) {
                this.ticketHttp.get("TicketScenic/GetScenicInfo?ticketScenicId=" + ticketScenicId).then(res => {
                    console.log(res);
            ...
            getScenicLog (ticketScenicId) {
                this.ticketHttp.get("TicketScenic/GetScenicLog?ticketScenicId=" + ticketScenicId).then(res => {
                    console.log(res);
                    if (res.data.isSuccess == true) {
