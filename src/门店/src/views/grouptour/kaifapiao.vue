<template>
  <div class="view">
    <el-row :gutter="20">
      <el-col :span="3" :offset="10" style="font-size:20px;">{{(disSettleClientName || '暂无名字')}}</el-col>
    </el-row>
    <div>
      <h4>发票详细</h4>
      <el-form
      ...
      >
        <el-form-item
          ...
          style="width:100%;"
        >
          <el-input v-model="fapiao_form.invoiceTitle" maxlength='100'></el-input>
          <el-button type="primary" size="small" @click="openDialog">选 择</el-button>
        </el-form-item>
        <el-form-item label="公司地址" class="form-inline-input addressSelect" prop="addressselect">
          <el-select
            v-model="fapiao_form.provinceId"
            class="addressSel"
            placeholder="- - - -请选择- - - -"
        ...
        <el-form-item class="form-inline-input" label="详细地址" style="width:100%;" prop="address">
          <el-input v-model="fapiao_form.address" maxlength='100'></el-input>
        </el-form-item>
        <el-form-item label="银行账号" class="form-inline-input" prop="account">
          <el-input v-model="fapiao_form.account" maxlength='100'></el-input>
        </el-form-item>
        <el-form-item label="开户行" class="form-inline-input" prop="bank">
          <el-input v-model="fapiao_form.bank" maxlength='100'></el-input>
        </el-form-item>
        <el-form-item label="公司税号" class="form-inline-input" prop="taxNumber">
          <el-input v-model="fapiao_form.taxNumber" maxlength='20'></el-input>
        </el-form-item>
        ...
        <el-form-item label="发票内容" class="form-inline-input" prop="content">
          <el-select v-model="fapiao_form.content" placeholder="请选择" multiple>
            <el-option
        ...
        <el-form-item label="领取方式" class="form-inline-input" style="width:100%;" prop="radio">
          <el-radio-group v-model="fapiao_form.radio">
        ...
        <el-form-item
          label="发票邮寄地址"
          class="form-inline-input addressSelect"
          v-if="fapiao_form.radio == 1"
          prop="toaddressselect"
        >
          <el-select
            v-model="fapiao_form.toprovinceId"
            class="addressSel"
            placeholder="- - - -请选择- - - -"
            @change="toprovinceChange"
          >
            <el-option
          ...
          <el-select
            v-model="fapiao_form.todistricId"
            class="addressSel"
            placeholder="- - - -请选择- - - -"
          >
            <el-option
      ...
      <div style="margin:15px 0;">
        ...
        <el-form ref="form1" :model="jilu_form" label-width="120px" size="small" :rules="rules1">
          ...
          <el-form-item class="form-inline-input" label="发票税费:" prop="taxAmount" style="width:30%">
            <el-input v-model="jilu_form.taxAmount" :disabled="true"></el-input>
          </el-form-item>
        </el-form>
      </div>
    ...
    <div style="text-align:center;margin-top:25px;">
      <el-button
        ...
        @click="saveInvoice"
        :loading="btn_loading"
        style="margin-right:20px;margin-left:-100px;"
      >保 存</el-button>
      <el-button class="button" type="primary" size="small" @click="goBack">取 消</el-button>
    </div>
    <el-dialog title="请选择发票信息" :visible.sync="dialog_show" :center="true">
      <el-table :data="selectData">
        <el-table-column type="index" label="序号" align="center"></el-table-column>
        <el-table-column label="发票抬头" align="center" prop="companyName"></el-table-column>
        <el-table-column label="开户行" align="center" prop="openingBank"></el-table-column>
        <el-table-column label="银行账号" align="center" prop="accountNo"></el-table-column>
        <el-table-column label="公司税号" align="center" prop="companyTax"></el-table-column>
        <el-table-column label="公司地址" align="center" prop="companyAddress"></el-table-column>
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <el-button type="text" size="mini" @click="handlerSelect(scope.row)">选 择</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-dialog>
    <el-dialog title="请选择邮寄信息" :visible.sync="postdialog_show" :center="true">
      <el-table :data="postData">
        <el-table-column type="index" label="序号" align="center"></el-table-column>
        <el-table-column label="收件人" align="center" prop="consignee"></el-table-column>
        <el-table-column label="收件地址" align="center" prop="postAddress" show-overflow-tooltip></el-table-column>
        <el-table-column label="联系电话" align="center" prop="phone"></el-table-column>
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <el-button type="text" size="mini" @click="showPostMsg(scope.row)">选 择</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-dialog>
...
export default {
  data() {
    ...
    let checktaxNumber = (rule, value, callback) =>{
      if(!value){
    ...
    var checkinvoiceAmount = (rule, value, callback) => {
      if (value == "" || value == 0) {
        callback(new Error("发票金额不能为空，为0"));
      } else if (!/^[+]?[\d]+(([\.]{1}[\d]+)|([\d]*))$/.test(value)) {
    ...
    var checkcontent = (rule, value, callback) => {
      if (this.fapiao_form.content.length == 0) {
    ...
    return {
      ...
      rules: {
        ...
        danwei: [{ required: true, validator: checkdanwei, trigger: "change" }],
        taxNumber: [
          { required: true, validator:checktaxNumber, trigger: "blur" }
        ],
        radio: [
          { required: true, message: "您还没选择领取方式", trigger: "blur" }
        ],
        toName: [{ required: true, validator: checktoName, trigger: "blur" }],
        toMobile: [{ required: true, validator: checkPhone, trigger: "blur" }],
        toaddress: [
          { required: true, validator: checktoaddress, trigger: "blur" }
        ],
        beizhu: [{ required: true, message: "您还没填写备注", trigger: "blur" }]
      },
      rules1: {
        invoiceAmount: [
  ...
  watch: {
    jilu_form: {
      handler: function(newVal, oldVal) {
          this.InvoiceType.forEach((item, index) => {
            if (item.id == this.fapiao_form.type) {
              newVal.taxAmount = Number(newVal.invoiceAmount * (item.taxPoint / 100)).toFixed(2);
            }
          });
      },
      deep: true
    ...
    fapiao_form: {
      handler: function(n, o) {
        this.InvoiceType.forEach((item, index) => {
          if (item.id == n.type) {
            this.jilu_form.taxAmount = Number(this.jilu_form.invoiceAmount * (item.taxPoint / 100)).toFixed(2);
          }
        });
      },
      deep:true
  ...
  methods: {
    ...
    provinceChange(val) {
      this.addressHttp
        .post("/AreaAddress/GetCity?ProvinceId=" + val)
        .then(res => {
          if (res.status == 200) {
            this.cityList = res.data;
            this.districList = [];
            (this.fapiao_form.cityId = ""), (this.fapiao_form.districId = "");
          }
        })
        .catch(e => {
          console.log(e);
    ...
    cityChange(val) {
      this.addressHttp
        .post("/AreaAddress/GetDistrict?CityId=" + val)
        .then(res => {
          // console.log(res)
          if (res.status == 200) {
        ...
        if (valid) {
          this.$refs.form1.validate(valid => {
            if (valid) {
              ...
              let contentStr = "";
              for (var i = 0; i < this.fapiao_form.content.length; i++) {
                contentStr += this.fapiao_form.content[i] + ",";
              }
              ...
              this.shopHttp
                .post("/Invoice/ApplyInvoice", {
                  ...
                  additionalAmount:
                    this.jilu_form.additionalAmount == ""
                      ? 0
                      : this.jilu_form.additionalAmount,
                  taxAmount:
                    this.jilu_form.taxAmount == ""
                      ? 0
                      : this.jilu_form.taxAmount,
                ...
                .then(res => {
                  if (res.data.isSuccess == true) {
                    ...
                    setTimeout(() => {
                      this.$router.push({ name: "fapiaoshenqingList" });
                    }, 500);
                  } else {
    ...
    openDialog() {
      ...
      this.shopHttp
        .get(
          "SettleClient/GetSettleClientInvoiceList?DisSettleClientId=" +
            this.DisSettleClientId
        )
        .then(res => {
          if (res.data.isSuccess == true) {
            this.selectData = res.data.objects;
          } else {
            this.$message({
              type: "info",
    ...
    //点击邮寄方式 选择按钮
    openPostDialog() {
      this.postdialog_show = true;
      this.shopHttp
        .get(
          "SettleClient/GetSettleClientPostalList?DisSettleClientId=" +
            this.DisSettleClientId
        )
        .then(res => {
          // console.log(res);
          if (res.data.isSuccess == true) {
            ...
            this.postData = res.data.objects;
          } else {
            this.$message({
              type: "info",
              message: "出错了" + res.data.message
    ...
    //展示发票信息
    handlerSelect(row) {
      this.fapiao_form.invoiceTitle = row.companyName;
      this.fapiao_form.address = row.address;
      this.fapiao_form.account = row.accountNo;
      this.fapiao_form.bank = row.openingBank;
      this.fapiao_form.taxNumber = row.companyTax;
      this.fapiao_form.provinceId = row.provinceID;
      this.addressHttp
        .post("/AreaAddress/GetCity?ProvinceId=" + row.provinceID)
        .then(res => {
          if (res.status == 200) {
            this.cityList = res.data;
            this.fapiao_form.cityId = row.cityID;
            this.addressHttp
              .post("/AreaAddress/GetDistrict?CityId=" + row.cityID)
              .then(res => {
                if (res.status == 200) {
                  this.districList = res.data;
                  this.fapiao_form.districId = row.districtID;
    ...
    //展示邮寄信息
    showPostMsg(row) {
      this.fapiao_form.toprovinceId = row.provinceID;
      this.fapiao_form.toaddress = row.address;
      this.fapiao_form.toMobile = row.phone;
      this.fapiao_form.toName = row.consignee;
      this.addressHttp
        .post("/AreaAddress/GetCity?ProvinceId=" + row.provinceID)
        .then(res => {
          if (res.status == 200) {
            this.tocityList = res.data;
            this.fapiao_form.tocityId = row.cityID;
            this.addressHttp
              .post("/AreaAddress/GetDistrict?CityId=" + row.cityID)
              .then(res => {
                if (res.status == 200) {
                  this.todistricList = res.data;
                  this.fapiao_form.todistricId = row.districtID;
                  this.postdialog_show = false;
                } else {
                  this.$message({
                    type: "info",
                    message: "出错了!" + res.data.message
      ...
      this.shopHttp
        ...
        .then(res => {
          ...
          if (res.data.isSuccess == true) {
            this.selectData = res.data.objects;
            res.data.objects.forEach(obj => {
              if (obj.ifDefault == true) {
                this.handlerSelect(obj);
              }
            });
          } else {
            this.$message({
  ...
  mounted() {
    this.shopHttp
      ...
      .then(res => {
        if (res.status == 200) {
          ...
          //计算未申请金额
          this.tableData[0].unApply =
            this.tableData[0].feeAmount - this.tableData[0].invoiceAmount;
          this.disSettleClientName = res.data.disSettleClientName;
          this.DisSettleClientId = res.data.disSettleClientId;
          this.getSettleClientInvoiceList();
          this.getPostDetail();
        } else {
          this.$message({
            type: "info",
    ...
    //获取省级行政单位
    this.addressHttp
      .post("/AreaAddress/GetProvince?CountryId=1")
      .then(res => {
        if (res.status == 200) {
