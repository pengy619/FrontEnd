    ...
    >
      ...
      <el-table
        ...
        <el-table-column v-if="isActivePayValidTime" label="剩余付款时间">
          <template slot-scope="scope">
            <span
              class="danger"
              v-if="scope.row.isActivePayValidTime&&scope.row.payStatus===0"
            >{{scope.row.showTime}}</span>
          </template>
        </el-table-column>
      </el-table>
      <el-row>
      ...
      <el-row class="text-center margin_top">
        ...
        <el-button
          ...
          :loading="false"
          :disabled="!form.IsNeedTheInvoice||!canSubmit"
        >支付并开发票</el-button>
...
export default {
  ...
  methods: {
    ...
    timer(duration, creationTime, index) {
      ...
      setInterval(() => {
        ...
        if (endTime > nowTime) {
          ...
          _this.tableData[
            index
          ].showTime = `${day}天${hour}小时${minute}分钟${second}秒`;
          betweenTime--;
        } else {
          time1 = 10000000000;
    ...
    getOrderPayList() {
      this.shopHttp
        .get(`SingleProductOrder/GetSingleProductOrderPaymentInfo?orderId=${this.form.OrderIds}`)
        .then(res => {
      ...
      if (amount >= 0) {
        for (var i = 0; i < this.tableData.length; i++) {
          ...
          if (amount >= 0) {
            ...
            this.tableData[i].receivedFee = Number(
              (this.tableData2[i].receivableFee + amount).toFixed(2)
            );
    ...
    postPayInfo(text) {
      this.shopHttp
        .post("CustomerFee/PaySingleProductOrder", this.form)
        .then(res => {
    ...
    handlePay(text) {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          this.shopHttp
            .post("SingleProductOrder/CheckPriceChange", {
              OrderIds: this.form.OrderIds
            }) //先检查是否有改价
            .then(res => {
              if (res.data.isSuccess) {
                this.$confirm(
                  "订单有改价变动在审核中，您确定以现订单金额进行付款吗",
                  "提示",
                  {
    ...
    back() {
      this.$router.push("/singleItem/singleOrderList");
    }
  }
};
</script>
