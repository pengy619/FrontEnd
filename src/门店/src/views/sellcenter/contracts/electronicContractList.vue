<template>
    <div class='view'>
        ...
        <el-table size="small" ref="multipleTable" :data="tableList" max-height="800" tooltip-effect="dark" style="width: 100%" v-loading = 'tableLoading'>
            ...
            <el-table-column size="small" prop="signeredStatus" label="签署状态" align="center" show-overflow-tooltip>
            </el-table-column>
    ...
    export default {
        ...
        methods:{
            getTable(pageIndex=1){
                ...
                this.shopHttp.get("Contract/GetElectronicContractList?"+
  ...
  handleViewContract(row){
    if(row.eleContract12301Url){
       window.open(row.eleContract12301Url) 
        ...
        let {href} = this.$router.resolve({path:'/sellcenter/electronicContractDetail',query:{contractNo:row.outContractNo}})
        window.open(href)
    }
