<template>
  <div class="view" v-loading="page_loading">
    <h4>
      ...
      ><a :href="fileUrl" style="color:#fff">下载行程信息</a></el-button>
    </h4>
    <el-form>
      ...
      <el-form-item label="线路套餐" size="small" label-width="120px">
        <el-select class="spanmargin" placeholder="请选择" v-model="linePackageId" @change="cityChange">
          <el-option
...
export default {
  ...
  methods: {
    ...
    queryData(packageId, size, index) {
      ...
      this.shopHttp
        .post(
         'ResourcePlatform/LinePriceDetail',{
           lineId:this.$route.query.lineId,
           packageId:packageId,
           Page:index,
           PageSize:size
    ...
    cashOrderdetail(data) {
      this.$router.push({path: "/sellcenter/reservationLine",
        query: {productId:this.lineId,dateOne:data.playDay,linePackageId:this.linePackageId}});
    },
    getPopoer(row) {
      //console.log(row);
    }
