            ...
            <el-tab-pane label="行程信息">
              <template>
                <div
                  ...
                  class="spanmargin"
                  v-for="(item,index) in detailInfo.lineIntros"
                  :key="index"
                >
...
export default {
  ...
  methods: {
    ...
    cashOrderdetail(data) {
      this.$router.push({
        ...
        query: {productId:this.$route.query.productId,productNo:this.$route.query.lineId,productCode:data.productCode}
      });
    },
    getPopoer(row) {
      //console.log(row);
  ...
  mounted() {
    this.shopHttp.get("ResourcePlatform/GetLineDetailForUTour?lineCode=" +this.$route.query.lineId).then(res => {
