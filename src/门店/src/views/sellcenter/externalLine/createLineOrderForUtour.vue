    ...
    <div>
      ...
      <div>
        ...
        <el-row>
          ...
          <el-col :span="20">
            <el-select
              v-model="lineProductCode"
              placeholder="请选择"
              size="small"
              @change="getLinePriceDateDetail(lineProductCode)"
            >
              <el-option
            ...
            <!-- <el-radio-group  v-model="lineProductCode" size="small">
              <el-radio 
                v-for="item in linePackageList"
                :key="item.code"
    ...
    <div>
      ...
      <el-table :data="table3" border style="width: 100%">
        ...
        <el-table-column label="操作" width="80">
          <template slot-scope="scope">
            <el-button @click="handleDelete2(scope.$index, scope.row)" type="danger" size="small" :disabled="personQuantity>=(scope.$index+1)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
  ...
  methods: {
    changeNum(newVal, oldVal, index) {
      if (newVal > oldVal) {
        ...
        this.table3.splice(this.table3.length-1, 1);
      }
    },
    //获取线路信息
    getlineData() {
        this.shopHttp.get("ResourcePlatform/GetLineDetailForUTour?lineCode=" +this.$route.query.productNo).then(res => {
            let routeDates = res.data.item.routeDates
            routeDates.forEach((item,index)=>{
                let obj = {};
                obj.name = item.productName+item.datestart+'出发';
                obj.code = item.productCode
                this.linePackageList.push(obj)
            })
            this.lineProductList = JSON.parse(JSON.stringify(res.data.item.routeDates))
            if(this.$route.query.productCode){
              this.lineProductCode = this.$route.query.productCode
              this.getLinePriceDateDetail(this.$route.query.productCode)
            }else{
              this.lineProductCode = this.linePackageList[0].code
              this.table1[0].SalesPrice = routeDates[0].adultPrice;
              this.table1[1].SalesPrice = routeDates[0].childPrice;
              this.table1[2].SalesPrice = routeDates[0].singleRoom;
              this.groupTime = res.data.item.routeDates[0].datestart
            }
            this.routeName = res.data.item.title;
            this.groupNum = res.data.item.lineCode;
            this.routeType =  res.data.item.function;
    ...
    //获取价格明细
    getLinePriceDateDetail(code) {
      this.lineProductList.forEach(item => {
        if (item.productCode == code) {
          //销售价
          this.table1[0].SalesPrice = item.adultPrice;
          this.table1[1].SalesPrice = item.childPrice;
          this.table1[2].SalesPrice = item.singleRoom;
          this.groupTime = item.datestart;
        }
      });
    },
    //添加游客信息
    ...
    onSubmit(form2, form3, type) {
      ...
      this.table3.forEach((item, index) => {
        if (item.Name == "" || item.Phone == "" || item.Sex =="") {
          isValidate = false;
          this.$message({ type: "info", message: "请输入姓名/性别/手机号码" });
        }
      });
      if (isValidate) {
        var obj = {};
        ...
        obj.ProductCode = this.lineProductCode;
            ...
            this.$refs["form3"].validate(valid => {
              if (valid) {
                if (this.personQuantity == 0) {
                  ...
                  this.shopHttp
                    .post("ResourcePlatform/CreateLineOrderForUtour", {
