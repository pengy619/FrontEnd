<template>
  <div class="view" v-loading="viewLoading">
    <h3 class="text-center">{{contractTemplateName}}</h3>
          ...
          <el-col :span="item.InputType!='T'?12:24" v-for="(item,index) in templateInfoList">
            <el-form-item :label="item.labelName">
              <el-input
                ...
                v-if="item.InputType=='A'"
                class="input_comm"
              ></el-input>
              <el-input
                type="textarea"
                v-model="item.InputDefaultValue"
                v-else-if="item.InputType=='B'"
                autosize
                class="input_comm"
              ></el-input>
              <el-radio-group v-model="item.InputDefaultValue" v-else-if="item.InputType=='C'">
                <el-radio
                  :label="item1"
                  v-for="(item1,index1) in item.InputDefaultValueList"
                >{{item1}}</el-radio>
              ...
              <div v-else-if="item.InputType=='T'">
                ...
                <el-table :data="item.tableArr" border>
                  <el-table-column label="操作" width="50" fixed>
                    <template slot-scope="scope">
                      <el-button
                        type="text"
                        @click="deleteRow(scope.row,scope.$index,index)"
                        size="mini"
                        v-if="scope.$index>0"
                      >删除</el-button>
                    </template>
...
export default {
  ...
  mounted() {
    this.shopHttp
      .get(
        "Contract/GetSignContractTemplate?OrderNo=" +
          this.$route.query.orderNo +
          "&OrderType="+
          this.form.OrderType+
          "&IsOfflineOrder=" +
          this.form.IsOfflineOrder +
          "&IsRemark=" +
          this.$route.query.isRemark
      )
      .then(res => {
        if (res.data.isSuccess == true) {
          ...
          if (objects.templateJsonValList.length > 0) {
            objects.templateJsonValList.forEach(item => {
              ...
              if (obj.InputType == "T") {
                ...
                obj.tableArr = [];
                obj.tableArr.push(JSON.parse(JSON.stringify(newItem)));
              }
              if (obj.InputType == "C") {
          ...
          this.contractTemplateName = objects.contractTemplateName;
  ...
  methods: {
    ...
    addRow(i) {
      let newItem = {};
      this.templateInfoList[i].InputDefaultValue.forEach((r, m) => {
        newItem["value" + m] = "";
      });
      this.templateInfoList[i].tableArr.push(newItem);
    },
    deleteRow(row, index, i) {
      this.templateInfoList[i].tableArr.splice(index, 1);
    },
    ...
    onSubmit(form, type) {
      ...
      this.templateInfoList.forEach(item => {
        var InputValues = item.InputDefaultValue;
        if (item.InputType == "T") {
          InputValues = [];
          item.tableArr.forEach(item2 => {
            let num = 0;
