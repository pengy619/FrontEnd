    ...
    <el-form ref="form" :model="form" :rules="rules" size="small" class="demo-form-inline" label-width="80px">
      ...
      <el-row>
        <el-form-item label="支付方式" prop="PayType">
          <el-radio-group v-model="form.PayType">
            ...
            <el-radio :label="16" v-show="isShow">供应商代收</el-radio>
      ...
      <el-row>
        <el-form-item label="支付金额" prop="PayAmount">
          <input type="text" style="position: fixed;bottom:999999999px;"/>
          <input type="password" style="position: fixed;bottom:999999999px;"/>
          <el-input-number :disabled="PayAmountDisabled || form.IsNeedFullPay" placeholder="请输入支付金额" :min="0" :max="maxMoney" :controls="false" class="input" v-model="form.PayAmount"></el-input-number>
          <el-button type="primary" @click="compute">计算</el-button>
          <span v-if="form.IsNeedFullPay" style="vertical-align: -webkit-baseline-middle;color:#999">提示：该订单需全额支付</span>
          <span v-if="PayAmountDisabled" style="vertical-align: -webkit-baseline-middle;color:#999">提示：部分收款无法输入金额</span>
        </el-form-item>
      </el-row>
      <div v-if="form.PayType != 16">
      ...
      <el-table :data="tableData" border style="width: 100%" class="margin_bottom" max-height="300px">
        ...
        <el-table-column label="业务类型">
          <template slot-scope="scope">
            {{'跟团游'}}
            <span class="out-window">{{scope.row.quantity}}</span>
          </template>
        </el-table-column>
        <el-table-column v-if="isActivePayValidTime" label="剩余付款时间">
          <template slot-scope="scope">
            <span class="danger" v-if="scope.row.isActivePayValidTime&&scope.row.payStatus===0">{{scope.row.showTime}}</span>
          </template>
        </el-table-column>
      </el-table>
      <el-row v-if="form.PayType != 16">
      ...
      <el-row class="danger text-center" v-show="!canSubmit">{{timeoutTips}}</el-row>
      <el-row class="text-center margin_top">
        <el-button size="small" type="primary" @click="handlePay()" :loading="loading1" :disabled="!canSubmit">支付</el-button>
        <el-button size="small" type="primary" @click="handlePay('kaifapiao')" :loading="loading2" :disabled="!form.IsNeedTheInvoice||!canSubmit||form.PayType == 16">支付并开发票</el-button>
...
<script>
  export default {
    ...
    methods: {
      ...
      //获取最小支付金额
      getMinDepositAmount(){
        this.shopHttp.get('SystemSetting/GetMinDepositAmount').then(res=>{
      ...
      timer(duration, creationTime, index) {
        ...
        setInterval(() => {
          ...
          if (endTime > nowTime) {
            ...
            _this.tableData[index].quantity = duration++
            _this.tableData[index].showTime = `${day}天${hour}小时${minute}分钟${second}秒`;
            betweenTime--;
          } else {
            time1 = 10000000000;
            if(_this.tableData[index].payStatus===0){
              _this.canSubmit = false;
            }else if(_this.tableData[index].payStatus===1){
      ...
      getOrderPayList() {
        this.shopHttp.post('GroupLine/GetPayGroupLineOrderList', { OrderIds: this.form.OrderIds })
          .then(res => {
            this.tableData = res.data.objects;
            if (this.tableData.length > 0) {
              this.tableData.forEach((item, index) => {
                if (item.isActivePayValidTime&&item.payStatus===0) {
            ...
            this.form.IsNeedFullPay = res.data.objects[0].payCount == 1;
      ...
      enableSupplierCollect(){
        this.shopHttp.get('/CustomerFee/EnableSupplierCollect').then(res=>{
          if(res.status == 200){
      ...
      postPayInfo(text) { 
        this.shopHttp
          .post("CustomerFee/PayGroupLineOrder", this.form)
          .then(res => {
