<template>
  <div id="box" class="view">
    ...
    <el-row class="danger text-center margin_top" v-show="!canSubmit">{{timeoutTips}}</el-row>
    <el-row :gutter="24" class=" text-center margin_top" >
        <el-button size="small" type="primary" @click="payinfoupdatesave()" :disabled="!canSubmit" >保 存</el-button>
        <el-button size="small" type="primary" @click="payinfoupdatesave('kaifapiao')" :disabled = 'needInvoice != 1||!canSubmit' >保存并开发票</el-button>
    ...
    methods:{
        ...
        cancel(){
            if(this.$route.query.status){
                 this.$router.push('/orderCenter/lineReservation')
            }else{
                this.$router.push('/sellcenter/linelist')
            }
